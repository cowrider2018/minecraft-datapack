...
execute at @e[type=minecraft:arrow,distance=..5,nbt=!{inGround:true}] run summon minecraft:zombie ~ ~ ~ {IsBaby:true,Tags:["goblin"],Attributes:[{Name:generic.follow_range,Base:100}],CustomNameVisible:1,CustomName:'[{"text":"哥布林","italic":false}]'}
team join goblin @e[type=zombie,tag=goblin]
